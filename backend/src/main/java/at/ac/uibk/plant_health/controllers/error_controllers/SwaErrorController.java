...
public class SwaErrorController implements ErrorController {
		...
		generateRedirectFromException (Exception exception) {
			return generateRedirectFromException (404, exception);
		}

		private RedirectResponse
		generateRedirectFromException (int status, Exception exception) {
		...
		@PublicEndpoint
		@RequestMapping (
			value  = AUTHENTICATION_ERROR_ENDPOINT,
		...
		@PublicEndpoint
		@RequestMapping (
			value  = TOKEN_EXPIRED_ERROR_ENDPOINT,
			method = {GET, POST, PUT, PATCH, DELETE}
		)
		@ResponseStatus (HttpStatus.UNAUTHORIZED)
		public RestResponseEntity
		...
		@PublicEndpoint
		@RequestMapping (
			value  = AUTHORIZATION_ERROR_ENDPOINT,
			method = {GET, POST, PUT, PATCH, DELETE}
		)
		...
		@PublicEndpoint
		@RequestMapping (
			value  = NOT_FOUND_ERROR_ENDPOINT,
			method = {GET, POST, PUT, PATCH, DELETE}
		)
		...
		@PublicEndpoint
		@RequestMapping (
			value = ERROR_ENDPOINT, method = {GET, POST, PUT, PATCH, DELETE}
		)
		...
		public void handleErrorManual (
			...
			if (exception instanceof
				...
				if (exception instanceof
					TokenExpiredException tokenExpiredException) {
					responseEntity = handleTokenExpiredError (
						request, response, tokenExpiredException
					);
				} else {
					responseEntity = handleAuthenticationError (
						request, response, authenticationException
					);
				}
			} else if (exception instanceof AccessDeniedException accessDeniedException) {
				responseEntity = handleAuthorizationError (
