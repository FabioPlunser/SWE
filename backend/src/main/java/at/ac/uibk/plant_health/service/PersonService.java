...
public class PersonService {
		...
		public Optional<Person> login (String username, String password) {
			...
			try {
				...
				log.warn (
					"Database Error while updating Token for User %s", person
				);
			}
		...
		findByUsernameAndToken (String username, UUID token) {
		 ...
		 */
		public boolean update (
			UUID personId, String username, Set<Permission> permissions,
			String password
		) {
			Optional<Person> maybePerson = findById (personId);
			return maybePerson
				.filter (
					person -> update (person, username, password, permissions)
				)
				.isPresent ();
		}
