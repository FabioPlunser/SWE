...
class TestLoginControllerGeneral {
  ...
  public void login() throws Exception {
    ...
    Set<GrantedAuthority> permissions =
        Set.of(Permission.USER, Permission.ADMIN);
    personService.create(
        new Person(username, StringGenerator.email(), password, permissions));
    ...
    mockMvc
        ...
        // then: status code 200 must be returned, token must be in body and
        // correct permissions must be returned
        .andExpectAll(
            status().isOk(), jsonPath("$.token").exists(),
            jsonPath("$.permissions").isArray(),
            jsonPath("$.permissions[*]")
                .value(Matchers.hasItems(Permission.USER.toString(),
  ...
  public void loginWrongPassword() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders
                     ...
                     .contentType(MediaType.APPLICATION_JSON))
        // then: status code 401 must be returned, token must not be contained
        // in body
