...
public class TestRouteAuthentication {
    ...
    private Person createUserWithToken(boolean alsoAdmin) {
            ...
            Set<GrantedAuthority> permissions = alsoAdmin ?
    Permission.adminAuthorities() : Permission.defaultAuthorities(); Person
    person = new Person(username, StringGenerator.email(), password,
    UUID.randomUUID(), permissions);
    assertTrue(personService.create(person), "Unable to create user");
    return person;
    }

    @SafeVarargs
    private HttpHeaders generateHttpHeaders(Pair<String, String> ...
    headers) { MultiValueMap<String, String> map = new
    LinkedMultiValueMap<>(); for (Pair<String, String> header : headers) {
    ...
    @Test
    public void testAnonymousAccessingAnonymousRouteWithOutCredentials()
    throws Exception {
            // given: No created Persons

            // when: Accessing an Anonymous Route without Credentials
    (anonymous)
    ...
    @Test
    public void testAnonymousAccessingAnonymousRouteWithCredentials() throws
    Exception {
            // given: A Person not stored in the database
            Person notSavedPerson = new Person("", "", "", UUID.randomUUID(),
    Set.of());
    ...
    //region Api Route Tests
    @Test
    public void testAnonymousAccessingApiRouteWithOutCredentials() throws
    Exception {
            // given: No created Persons
            ...
            mockMvc.perform(MockMvcRequestBuilders.get(TEST_API_ENDPOINT())
                                            .contentType(MediaType.APPLICATION_JSON)
                            // then: Expect an Authentication Exception resulting in a
    401 Error Code
            ).andExpectAll(
                            status().is(Matchers.allOf(Matchers.greaterThan(300),
    Matchers.lessThan(500)))
            );
    }

    @Test
    public void testAnonymousAccessingApiRouteWithCredentials() throws
    Exception {
            // given: A Person not stored in the database
            Person notSavedPerson = new Person("", "", "", UUID.randomUUID(),
    Set.of());
    ...
    @Test
    public void testAnonymousAccessingApiAdminRouteWithOutCredentials()
    throws Exception {
            // given: No created Persons
    ...
    @Test
    public void testAnonymousAccessingApiAdminRouteWithCredentials() throws
    Exception {
            // given: No created Persons
            Person notSavedPerson = new Person("", "", "", UUID.randomUUID(),
    Set.of());
    ...
    AuthGenerator.generateToken(person))
    .contentType(MediaType.APPLICATION_JSON)
                            // then: Expect an Authorization Exception resulting in a
    403 Error Code
            ).andExpectAll(
                            status().is(Matchers.allOf(Matchers.greaterThan(300),
    Matchers.lessThan(500)))
    ...
    @Test
    public void testAnonymousAccessingAdminRouteWithOutCredentials() throws
    Exception {
            // given: No created Persons
    ...
    @Test
    public void testAnonymousAccessingAdminRouteWithCredentials() throws
    Exception
    {
            // given: No created Persons
            ...
            // when: Accessing an Admin Page with Credentials (anonymous)
            mockMvc.perform(MockMvcRequestBuilders.get(TEST_ADMIN_ENDPOINT())
                                            .cookie(AuthGenerator.jwtTokenToCookies(new
    JwtToken("", UUID.randomUUID())))
    .contentType(MediaType.APPLICATION_JSON)
                            // then: Expect an Authentication Exception resulting in a
    401 Error Code
            ).andExpectAll(
    ...
    private void setTokenCreationDate(Authenticable authenticable,
    LocalDateTime creationDate) throws NoSuchFieldException { Field
    tokenCreationDateField =
    Authenticable.class.getDeclaredField("tokenCreationDate");
            tokenCreationDateField.setAccessible(true);
            ReflectionUtils.setField(tokenCreationDateField, authenticable,
    creationDate);
    }
    ...
    public void testExpiredToken() throws Exception {
            ...
            setTokenCreationDate(person,
    LocalDateTime.now().minus(tokenExpirationDuration));
