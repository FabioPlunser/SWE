...
public interface PersonRepository extends CrudRepository<Person, UUID> {
  ...
  default<S extends Person> S updateToken(S person) {
    return updateToken(person.getPersonId(), person.getToken(),
                       person.getTokenCreationDate()) == 1
        ? person
        : null;
  }
  ...
  default<S extends Person> S updateUserDetails(S person) {
    return updateUserDetails(person.getPersonId(), person.getUsername(),
                             person.getPermissions()) == 1
        ? person
        : null;
  }
