...
public class TestRouteAuthentication {
  ...
  Permission.adminAuthorities() : Permission.defaultAuthorities(); Person person
  = new Person(username, StringGenerator.email(), password, UUID.randomUUID(),
  permissions); assertTrue(personService.create(person), "Unable to create
  user"); return person;
  }
  ...
  @Test
  public void testAnonymousAccessingAnonymousRouteWithOutCredentials() throws
  Exception {
  ...
  public void testUserAccessingAnonymousRoute() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.get(TEST_ANONYMOUS_ENDPOINT())
                      .header(HttpHeaders.AUTHORIZATION,
  AuthGenerator.generateToken(person)) .contentType(MediaType.APPLICATION_JSON)
              // then: Expect that the Page is returned
      ).andExpectAll(
              status().isOk()
      );
  ...
  Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.get(TEST_API_ENDPOINT())
                      .contentType(MediaType.APPLICATION_JSON)
              // then: Expect an Authentication Exception resulting in a 401
  Error Code
      ).andExpectAll(
  ...
  @Test
  public void testAnonymousAccessingApiAdminRouteWithOutCredentials() throws
  Exception {
      // given: No created Persons
  ...
  public void testUserAccessingApiAdminRoute() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.get(TEST_API_ADMIN_ENDPOINT())
                      .header(HttpHeaders.AUTHORIZATION,
  AuthGenerator.generateToken(person)) .contentType(MediaType.APPLICATION_JSON)
              // then: Expect an Authorization Exception resulting in a 403
  Error Code
      ).andExpectAll(
              status().is(Matchers.allOf(Matchers.greaterThan(300),
  Matchers.lessThan(500)))
  ...
  public void testAnonymousAccessingAdminRouteWithCredentials() throws Exception
  {
      ...
      // when: Accessing an Admin Page with Credentials (anonymous)
      mockMvc.perform(MockMvcRequestBuilders.get(TEST_ADMIN_ENDPOINT())
                      .cookie(AuthGenerator.jwtTokenToCookies(new JwtToken("",
  ...
  private void setTokenCreationDate(Authenticable authenticable, LocalDateTime
  creationDate) throws NoSuchFieldException { Field tokenCreationDateField =
