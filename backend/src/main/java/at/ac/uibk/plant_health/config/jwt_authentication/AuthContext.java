...
public class AuthContext {
		 ...
		 * authenticated. A User is authenticated if he is logged-in with
		 * Username and Password (and in turn with his Token). Otherwise, the
		...
		/**
		 * Checks whether the currently logged-in user has the required
		...
		anyPermission (Collection<GrantedAuthority> required) {
			return getCurrentUser ()
				.map (Authenticable::getPermissions)
				.map (
					permissions
					-> required.stream ().anyMatch (permissions::contains)
				)
				.orElse (false);
		}
		 ...
		 */
		public static boolean
		allPermissions (Collection<GrantedAuthority> required) {
			return getCurrentUser ()
				.map (Authenticable::getPermissions)
				.map (
					permissions
					-> required.stream ().allMatch (permissions::contains)
				)
				.orElse (false);
		}

		/**
		 * Checks whether the currently logged-in user does not have the
		 * required Permission. This method also works if the user is not
		 * logged-in or anonymous (and will return false in those cases).
		 *
		...
		/**
		 * Checks whether the currently logged-in user does not have the
		 * required Permissions. This method also works if the user is not
		 * logged-in or anonymous (and will return false in those cases).
		 *
