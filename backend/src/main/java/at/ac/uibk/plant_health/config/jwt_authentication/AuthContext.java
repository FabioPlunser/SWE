...
public class AuthContext {
   ...
   * A User is authenticated if he is logged-in with Username and Password (and
   * in turn with his Token). Otherwise, the Request is anonymous.
   *
   * @return true if a logged-in user made the request, false if the request is
   *     anonymous or if the Authentication failed.
   */
  public static boolean isAuthenticated() {
    // Get the Authentication set by the FilterChain.
    Authentication authentication =
        SecurityContextHolder.getContext().getAuthentication();
    // Ensure that the Authentication succeeded by ensuring it is not null and
    // that the Authentication is not anonymous.
   ...
   *
   * @return The AuthenticationToken sent with the current request (if a valid
   *     one was sent).
   * @see UsernamePasswordAuthenticationToken
   */
  private static Optional<UsernamePasswordAuthenticationToken>
  getAuthentication() {
    if (!isAuthenticated())
   ...
   *
   * @return The Authenticable whose credentials were sent with the request (if
   ...
   *
   * @return The Person whose credentials were sent with the request (if valid
  ...
  /**
   * Get the Token that was sent with the request (either as a Bearer-, or
   * Cookie-Token).
   *
   ...
   * This method also works if the user is not logged-in or anonymous (and will
  ...
  /**
   * Checks whether the currently logged-in user has any of the required
   * Permissions. This method also works if the user is not logged-in or
   * anonymous (and will return false in those cases).
   *
  ...
  /**
   * Checks whether the currently logged-in user has all the required
   * Permissions. This method also works if the user is not logged-in or
   * anonymous (and will return false in those cases).
   *
   * @param required The Permissions that the User needs to have.
  ...
  /**
   * Checks whether the currently logged-in user does not have the required
   * Permission. This method also works if the user is not logged-in or
   * anonymous (and will return false in those cases).
   *
   * @param permission The Permission that the User needs to miss.
   * @return true if the user does not have the required Permissions, false
   *     otherwise.
   */
   ...
   * @return true if the user does not have any of the required Permissions,
   *     false otherwise.
   */
  public static boolean
  missingPermissions(Collection<GrantedAuthority> permissions) {
