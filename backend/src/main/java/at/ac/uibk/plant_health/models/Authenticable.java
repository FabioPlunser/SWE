...
public abstract class Authenticable
	implements UserDetails, CredentialsContainer {
		...
		protected Authenticable (
			String username, String password, UUID token,
			Set<GrantedAuthority> permissions
		) {
			this (
				null, username, password, false, token, LocalDateTime.now (),
				permissions
			);
		}

		protected Authenticable (
			String username, String password, Set<GrantedAuthority> permissions
		) {
		...
		@JsonProperty (access = JsonProperty.Access.READ_WRITE)
		@Column (
			name = "token_creation_date", nullable = true,
		...
		@ElementCollection (
			targetClass = Permission.class, fetch = FetchType.EAGER
		)
		@CollectionTable (
			name = "permission", joinColumns = @JoinColumn (name = "auth_id")
		)
		@Fetch (FetchMode.SELECT)
		private Set<GrantedAuthority> permissions =
			Permission.defaultAuthorities ();
		...
		public boolean equals (Object o) {
			return (this == o) ||
				((o instanceof Authenticable a) && (this.id != null) &&
				 (this.id.equals (a.id)));
		}
		...
		public void setToken (UUID token) {
			...
			this.tokenCreationDate =
				token != null ? LocalDateTime.now () : null;
		}
		// endregion
