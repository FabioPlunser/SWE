...
public class TestPersonServiceGeneral {
		...
		public void saveAndGetPersons () {
			...
			for (Person person : savedPersons) {
				...
				assertEquals (
						person.getPersonId (), foundPerson.getPersonId (), "Wrong id of " + person
				);
				assertEquals (
				...
				assertEquals (
						person.getEmail (), foundPerson.getEmail (), "Wrong email of " + person
				);
				assertEquals (
		...
		public void createTwoIdenticalUsernames () {
			...
			Person person	= new Person (
					  username, StringGenerator.email (), StringGenerator.password (), Set.of ()
			  );
		...
		public void updatePerson () {
			...
			assertTrue (
					maybeOldCredentialsPerson.isEmpty (), "Could still login with old credentials"
			);
			assertEquals (
		...
		public void updatePersonViaCreate () {
			...
			assertTrue (
					personService.login (username, password).isPresent (), "Could not login anymore"
			);
			assertEquals (email, foundPerson.getEmail (), "Email was changed");
			assertEquals (
					Set.of (), foundPerson.getPermissions (), "Permissions have been changed"
			);
		}
