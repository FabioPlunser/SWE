...
public class SwaErrorController implements ErrorController {
  ...
  @Autowired private EndpointMatcherUtil endpointMatcherUtil;
  ...
  private RedirectResponse generateRedirectFromException(int status,
                                                         Exception exception) {
  ...
  @ReadOperation
  @PublicEndpoint
  @RequestMapping(value = AUTHENTICATION_ERROR_ENDPOINT,
  ...
  @ReadOperation
  @PublicEndpoint
  @RequestMapping(value = TOKEN_EXPIRED_ERROR_ENDPOINT,
                  method = {GET, POST, PUT, PATCH, DELETE})
  @ResponseStatus(HttpStatus.UNAUTHORIZED)
  public RestResponseEntity
  handleTokenExpiredError(HttpServletRequest request,
                          HttpServletResponse response,
  ...
  @ReadOperation
  @PublicEndpoint
  @RequestMapping(value = AUTHORIZATION_ERROR_ENDPOINT,
                  method = {GET, POST, PUT, PATCH, DELETE})
  @ResponseStatus(HttpStatus.FORBIDDEN)
  public RestResponseEntity
  handleAuthorizationError(HttpServletRequest request,
                           HttpServletResponse response,
  ...
  @ReadOperation
  @PublicEndpoint
  @RequestMapping(value = NOT_FOUND_ERROR_ENDPOINT,
                  method = {GET, POST, PUT, PATCH, DELETE})
  @ResponseStatus(HttpStatus.NOT_FOUND)
  public RestResponseEntity
  handleNotFoundError(HttpServletRequest request,
                      HttpServletResponse response) {
    return MessageResponse.builder()
  ...
  @ReadOperation
  @PublicEndpoint
  @RequestMapping(value = ERROR_ENDPOINT,
                  method = {GET, POST, PUT, PATCH, DELETE})
  @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
  public RestResponseEntity
  handleError(HttpServletRequest request, HttpServletResponse response,
              Exception exception) {
    return MessageResponse.builder()
        .success(false)
  ...
  public void handleErrorManual(HttpServletRequest request,
                                HttpServletResponse response,
    ...
    if (exception instanceof AuthenticationException authenticationException) {
      if (exception instanceof TokenExpiredException tokenExpiredException) {
        responseEntity =
            handleTokenExpiredError(request, response, tokenExpiredException);
      } else {
        responseEntity = handleAuthenticationError(request, response,
                                                   authenticationException);
      }
    } else if (exception instanceof
               AccessDeniedException accessDeniedException) {
      responseEntity =
          handleAuthorizationError(request, response, accessDeniedException);
    } else {
    ...
    try {
      ...
      responseEntity.getHeaders().forEach(
          (name,
           values) -> values.forEach(value -> response.addHeader(name, value)));

      // Write the Body of the Request
      String responseBody =
          SerializationUtil.serializeJSON(responseEntity.getBody());
