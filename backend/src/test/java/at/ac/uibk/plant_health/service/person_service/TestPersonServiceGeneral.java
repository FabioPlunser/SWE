...
public class TestPersonServiceGeneral {
		...
		public void saveAndGetPersons () {
			...
			for (int i = 0; i < numberOfDemoPersons; i++) {
				Person person = new Person (
					StringGenerator.username (), StringGenerator.email (),
			...
			for (Person person : savedPersons) {
				assertTrue (
					foundPersons.contains (person),
					"Could not find person " + person
				);
				Person foundPerson =
					foundPersons.get (foundPersons.indexOf (person));
				assertEquals (
					person.getPersonId (), foundPerson.getPersonId (),
					"Wrong id of " + person
				);
				assertEquals (
					person.getUsername (), foundPerson.getUsername (),
					"Wrong username of " + person
				);
				assertEquals (
					person.getEmail (), foundPerson.getEmail (),
		...
		public void createTwoIdenticalUsernames () {
			...
			Person person	= new Person (
				  username, StringGenerator.email (), StringGenerator.password (),
				  Set.of ()
			  );
		...
		public void getPersonById () {
			...
			assertTrue (
				personService.create (person), "Unable to create user for test"
			);
		...
		public void updatePerson () {
			...
			Set<Permission> newPermissions =
				Set.of (Permission.ADMIN, Permission.USER);
			assertTrue (
				personService.update (
					person, newUsername, newPassword, newPermissions
				),
			...
			assertTrue (
				maybePerson.isPresent (), "Could not login with new credentials"
			);
			Optional<Person> maybeOldCredentialsPerson =
				personService.login (username, password);
			assertTrue (
				maybeOldCredentialsPerson.isEmpty (),
		...
		public void updatePersonViaCreate () {
			...
			assertEquals (
				username, foundPerson.getUsername (), "Name was changed"
			);
			assertTrue (
				personService.login (username, password).isPresent (),
				"Could not login anymore"
			);
			assertEquals (email, foundPerson.getEmail (), "Email was changed");
			assertEquals (
				Set.of (), foundPerson.getPermissions (),
		...
		public void deletePerson () {
			...
			assertFalse (
				personService.getPersons ().contains (person),
