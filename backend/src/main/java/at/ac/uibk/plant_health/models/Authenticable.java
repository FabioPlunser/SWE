...
public abstract class Authenticable
  ...
  // region Constructors
  protected Authenticable(String username, String password, UUID token,
                          Set<GrantedAuthority> permissions) {
    this(null, username, password, false, token, LocalDateTime.now(),
         permissions);
  }
  ...
  // region Fields
  @Id
  // NOTE: Classes that extend this should create a Getter with @JsonInclude to
  // rename the ID.
  @JsonIgnore
  ...
  //       Constructor, the Password is not hashed (see CTOR). But if it is
  //       pulled from the database, the Password is assumed to be hashed (see
  //       default Value).
  @Transient
  @JsonIgnore
  @Builder.Default
  ...
  public boolean equals(Object o) {
    return (this == o) || ((o instanceof Authenticable a) &&
                           (this.id != null) && (this.id.equals(a.id)));
  }
