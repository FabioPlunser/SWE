...
public class TestPersonServiceGeneral {
  ...
  public void saveAndGetPersons() {
    ...
    for (int i = 0; i < numberOfDemoPersons; i++) {
      Person person =
          new Person(StringGenerator.username(), StringGenerator.email(),
                     StringGenerator.password(), Set.of());
      savedPersons.add(person);
      assertTrue(personService.create(person),
    ...
    for (Person person : savedPersons) {
      assertTrue(foundPersons.contains(person),
                 "Could not find person " + person);
      Person foundPerson = foundPersons.get(foundPersons.indexOf(person));
      assertEquals(person.getPersonId(), foundPerson.getPersonId(),
                   "Wrong id of " + person);
      assertEquals(person.getUsername(), foundPerson.getUsername(),
                   "Wrong username of " + person);
      assertEquals(person.getEmail(), foundPerson.getEmail(),
                   "Wrong email of " + person);
  ...
  public void createTwoIdenticalUsernames() {
    ...
    Person person = new Person(username, StringGenerator.email(),
                               StringGenerator.password(), Set.of());
    assertTrue(personService.create(person), "Unable to create user");

    // when: creating a second user with exactly the same username
    Person duplicatePerson = new Person(username, StringGenerator.email(),
                                        StringGenerator.password(), Set.of());
  ...
  public void getPersonById() {
    ...
    assertEquals(person, maybePerson.get(),
                 "Got user " + maybePerson.get() + " when user " + person +
                     " was expected");
  }
  ...
  public void updatePerson() {
    ...
    // then: logging in should be possible with new credentials only and other
    // attributes must be correct
    Optional<Person> maybePerson =
        personService.login(newUsername, newPassword);
    assertTrue(maybePerson.isPresent(), "Could not login with new credentials");
    Optional<Person> maybeOldCredentialsPerson =
        personService.login(username, password);
    assertTrue(maybeOldCredentialsPerson.isEmpty(),
               "Could still login with old credentials");
  ...
  public void updatePersonViaCreate() {
    ...
    assertFalse(personService.create(person),
                "Changed person got created again");
    Optional<Person> maybePerson = personService.findById(id);
    assertTrue(maybePerson.isPresent(),
               "Unable to find original person in repository");
    Person foundPerson = maybePerson.get();
    assertEquals(username, foundPerson.getUsername(), "Name was changed");
    assertTrue(personService.login(username, password).isPresent(),
               "Could not login anymore");
    assertEquals(email, foundPerson.getEmail(), "Email was changed");
    assertEquals(Set.of(), foundPerson.getPermissions(),
  ...
  public void deletePerson() {
    ...
    assertTrue(personService.delete(person.getPersonId()),
               "Could not delete user");

    // then: user should not exist in database anymore
    assertFalse(personService.getPersons().contains(person),
