...
public class TestPersonServiceSecurity {
		...
		public void loginWithValidCredentials () {
			...
			Person person			 = new Person (
				   username, StringGenerator.email (), password, Set.of ()
			   );
			assertTrue (
				personService.create (person), "Unable to create user " + person
			);
			for (int i = 0; i < numberOfOtherPersons; i++) {
				assertTrue (
					personService.create (new Person (
						"otherPerson-TestLoginWithValidCredentials-" + (i + 1),
						StringGenerator.email (), StringGenerator.password (),
						Set.of ()
					)),
		...
		public void loginWithInvalidCredentials () {
			...
			assertTrue (
				maybePersonUsernameWrong.isEmpty (),
				"Could login with wrong username"
			);
			assertTrue (
				maybePersonPasswordWrong.isEmpty (),
		...
		public void getPersonByToken () {
			...
			assertTrue (
				maybePersonByToken.isPresent (), "Did not find user by token"
			);
			assertEquals (
				person, maybePersonByToken.get (),
				"Got user " + maybePersonByToken.get () + " when user " +
					person + " was expected"
			);
		}
