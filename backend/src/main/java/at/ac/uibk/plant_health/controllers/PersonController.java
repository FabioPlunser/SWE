...
public class PersonController {
		...
		public RestResponse create (
			...
			Person person =
				new Person (username, email, password, (Set) permissions);
		...
		public RestResponse updateUser (
			...
			if (personService.update (
					personId, username, permissions, password
				))
				return new MessageResponse (
					true, "User " + personId + " updated successfully!"
				);

			return new MessageResponse (
				false,
				"Could not update User " + personId + " - User does not exist!"
			);
		}
		// endregion
		...
		public RestResponse deleteUser (@RequestParam ("personId"
		) final UUID personId) {
			if (!personService.delete(personId))
				return new MessageResponse (
					false, "Could not delete User " + personId +
							   " - User does not exist!"
				);

			return new MessageResponse (
				true, "User " + personId + " deleted successfully!"
			);
		}
		// endregion
