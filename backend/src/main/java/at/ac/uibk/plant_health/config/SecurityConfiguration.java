...
public class SecurityConfiguration {
  ...
  @Autowired private StartupConfig.Profile profile;
  ...
  private RequestMatcher getPublicEndpointMatcher() {
    return new OrRequestMatcher(
        getPublicEndpointStream().toArray(RequestMatcher[] ::new));
  }
  ...
  private Stream<String> getPublicEndpointStrings() {
    // NOTE: This is a workaround because Spring Security does not define an
    // Annotation
  ...
  @Value("${springdoc.api-docs.path}") private String swaggerDocPath;

  @Value("${springdoc.swagger-ui.path}") private String swaggerApiPath;
  ...
  private Stream<String> getSwaggerEndpointStrings() {
    // Enable the Swagger Endpoints in the Development Environment
    if (this.profile == StartupConfig.Profile.DEBUG) {
      return Stream.of(swaggerDocPath, swaggerApiPath,
  ...
  filterChain(HttpSecurity http, AuthenticationEntryPoint entryPoint,
    ...
    RequestMatcher protectedMappings =
        new NegatedRequestMatcher(publicMappings);
