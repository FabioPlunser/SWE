...
public class PersonController {
  ...
  public RestResponse register(@RequestParam("username") final String username,
    ...
    Person person = new Person(username, email, password, token,
   ...
   * @return A RestResponse indicating whether the operation was successful or
   *     not.
   */
  ...
  public RestResponse updateUser(
      ...
      @RequestParam(name = "permissions",
                    required = false) final Set<Permission> permissions,
      @RequestParam(name = "password",
                    required = false) final String password) {
    if (personService.update(personId, username, permissions, password))
      return new MessageResponse(true,
                                 "User " + personId + " updated successfully!");

    return new MessageResponse(false, "Could not update User " + personId +
                                          " - User does not exist!");
  }
  // endregion
  ...
  public RestResponse
  deleteUser(@RequestParam("personId") final UUID personId) {
    if (!personService.delete(personId))
      return new MessageResponse(false, "Could not delete User " + personId +
                                            " - User does not exist!");

    return new MessageResponse(true,
                               "User " + personId + " deleted successfully!");
  }
  // endregion
  ...
  /**
   * Endpoint for Admins to get all possible Permission so that they don't need
   * to be changed manually on frontend.
   *
  ...
  public RestResponse getAllPermissions() {
    return new ListResponse<>(
        Stream.of(Permission.values()).map(Enum::name).toList());
  }
  // endregion
}
